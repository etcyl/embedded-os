...
int main()
{
    ...
    char user_in[BUFF_LEN], EXIT_CMD[BUFF_LEN], PARSE_CMD[BUFF_LEN], HISTORY_CMD[BUFF_LEN], BANG_CMD[BUFF_LEN], INVAL_REF[BUFF_LEN], temp[3];
    char** BUFFER;
          ...
          if(strcmp(user_in, HISTORY_CMD) == 0)
             {
              ...
              else
              {
                ...
                {
                  if(strcmp(BUFFER[i], INVAL_REF) == 0)
                    printf("\n");
                  else
                    printf("%s", BUFFER[i]);
                }
                        ...
                        if(isdigit(user_in[6]))
                        {
                            if((int) user_in[6] - '0' > line_count)
                            ...
                            else
                              {
                               ...
                               strcpy(BUFFER[line_count], INVAL_REF);
                              }
                        }
                      }
                     }
