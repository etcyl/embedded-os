...
int main()
{
    ...
    char user_in[BUFF_LEN], EXIT_CMD[BUFF_LEN], PARSE_CMD[BUFF_LEN], HISTORY_CMD[BUFF_LEN], BANG_CMD[BUFF_LEN], INVAL_REF[BUFF_LEN], NONE[BUFF_LEN], temp[3];
    char** BUFFER, head;
    ...
    for(i = 0; i < CIRC_BUFF_LEN; ++i)
       strcpy(BUFFER[i], NONE);
             ...
             if(isdigit(user_in[2]))
               {
                   for(i = 0; i < 2; ++i)
                      temp[i] = user_in[i +1];
                   i = atoi(temp);
                  if( i > line_count || i <= 0 || i >= 5)
                    printf("JUST KIDDING you can't do that; not a valid line number (...yet)\n");
                  else if(strcmp(BUFFER[i], NONE) == 0)
